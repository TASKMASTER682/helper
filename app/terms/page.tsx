...
export default function TermsOfService() {
  ...
  return (
    <div className="min-h-screen bg-ink-950 text-ink-100 font-body selection:bg-red-500/30">
      <nav className="fixed top-0 w-full z-50 border-b border-ink-600/50 bg-ink-950/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-500 to-red-700 flex items-center justify-center">
              <Brain className="w-6 h-6 text-white" />
            </div>
            <div className="font-display text-xl font-bold text-ink-50 tracking-tight">UPSC-POS</div>
          </Link>
          <button 
            onClick={() => router.back()}
      ...
      <main className="pt-40 pb-20 px-6 max-w-4xl mx-auto">
        <div className="mb-12">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-red-500/20 bg-red-500/5 text-red-400 text-[10px] font-mono tracking-[0.2em] uppercase mb-6">
            <Scale className="w-3.5 h-3.5" /> Governance Framework
          </div>
          <h1 className="text-4xl md:text-6xl font-black text-ink-50 uppercase tracking-tighter mb-4">Terms of Service</h1>
          <p className="text-ink-500 text-sm font-mono uppercase tracking-widest">Version 1.0 • Last Synchronized: April 2026</p>
        </div>

        <div className="space-y-12">
          <section className="glass-card p-8 border-ink-600 bg-ink-900/20">
            <h2 className="text-xl font-black text-ink-50 uppercase tracking-tight mb-4 flex items-center gap-3">
              <Zap className="w-5 h-5 text-red-500" /> Acceptable Use
            </h2>
          ...
          <section>
            <h2 className="text-xl font-black text-ink-50 uppercase tracking-tight mb-4">Account Integrity</h2>
          ...
          <section>
            <h2 className="text-xl font-black text-ink-50 uppercase tracking-tight mb-4">Platform Availability</h2>
            <p className="text-ink-400 leading-relaxed">
      ...
      <footer className="border-t border-ink-600/50 py-12 text-center">
